...
export default function DoctorMedicationsScreen() {
  ...
  const handleRetractMedication = async (item: Medication) => {
    ...
    try {
      ...
      await notifyRecordChange(amka, loggedInDoctorAmka, 'Φάρμακα', 'retracted', item.title, item.url);
    } catch (error: any) {
  ...
  const handleUndoRetractMedication = async (item: Medication) => {
    ...
    try {
      ...
      await notifyRecordChange(amka, loggedInDoctorAmka, 'Φάρμακα', 'restored', item.title, item.url);
    } catch (error: any) {
