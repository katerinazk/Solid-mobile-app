...
export default function DoctorDiagnoseisScreen() {
  ...
  const handleRetractDiagnosis = async (item: Diagnosis) => {
    ...
    try {
      ...
      await notifyRecordChange(amka, loggedInDoctorAmka, 'Διαγνώσεις', 'retracted', item.title);
    } catch (error: any) {
  ...
  const handleUndoRetractDiagnosis = async (item: Diagnosis) => {
    ...
    try {
      ...
      await notifyRecordChange(amka, loggedInDoctorAmka, 'Διαγνώσεις', 'restored', item.title);
    } catch (error: any) {
