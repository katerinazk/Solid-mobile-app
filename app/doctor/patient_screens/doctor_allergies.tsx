...
export default function DoctorAllergiesScreen() {
  ...
  const handleRetractAllergy = async (item: Allergy) => {
    ...
    try {
      ...
      await notifyRecordChange(amka, loggedInDoctorAmka, 'Αλλεργίες', 'retracted', item.title);
    } catch (error: any) {
  ...
  const handleUndoRetractAllergy = async (item: Allergy) => {
    ...
    try {
      ...
      await notifyRecordChange(amka, loggedInDoctorAmka, 'Αλλεργίες', 'restored', item.title);
    } catch (error: any) {
